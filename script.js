...
var size = 10;
for (var i = 0; i < 10; i++) {
  pages[i] = data.slice(size * i, size * i + 10);
}

var domPage = document.createElement("div");
domPage.className = "dom";
domPage.innerText = "DOM PAGINATION";
...
function displayPage(index) {
  current = index;
  pages[index - 1].forEach((x) => {
    var trow = document.createElement("tr");
    var td1 = document.createElement("td");
    var td2 = document.createElement("td");
    var td3 = document.createElement("td");
    td1.innerText = x.id;
    td2.innerText = x.name;
...
function createButton(text, index) {
  ...
  btn.addEventListener("click", function () {
    tbody.innerHTML = "";
    if (text === "Previous") prevPage();
    else displayPage(index);
  });
}
...
function prevPage() {
  ...
  } else displayPage(current - 1);
}

document.body.append(domPage, table, buttons);
